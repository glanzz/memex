...
class Cache:
  ...
  def retrieve_cache(cls, key):
    ...
    if cache_info:
      ...
      if datetime.datetime.utcfromtimestamp(cache_info["expire"]) > datetime.datetime.utcnow():
        Logger.debug("Cached data valid !")
        body = ''
        with open(cls.__get_file_path(file_name=cache_info["name"]), "r") as cachefile:
          body = cachefile.read()
  ...
  @classmethod
  def cache_file(self, key, file_content, encoding_type, cache_time=DEFAULT_CACHE_TIME):
    ...
    if self.__CACHE_INDEX.get(key):
      ...
      with open(self.__get_file_path(file_name=file_name), "x") as cache_file:
