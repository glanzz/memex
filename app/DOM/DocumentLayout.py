...
class DocumentLayout:
    ...
    def layout(self):
        child = Layout(
            self.node, parent=self, previous=None
        )
        self.width = self.window_width - (2*HSTEP) # 2 HSTEPs is gutter space at the ends 
        self.x = HSTEP
        self.y = VSTEP
        self.children.append(child)
        child.layout()
        self.height = child.height # This value is set by accumlating all the previous values of children
