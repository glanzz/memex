...
class HTMLParser:
  ...
  def parse(self):
    buffer = ""
    show_data = self.body.decode(self.encoding if self.encoding else "utf-8") if self.encoding != None else self.body
    in_tag = False
  ...
  def add_text(self, text):
    if text.isspace(): return # Ignores whitespace texts
  ...
  def add_tag(self, tag):
    if tag.startswith("!"):return # Ignore ! doctype and comments
    ...
    if len(parts) > 1:
      for attribute in parts[1:]:
        if "=" in attribute:
          ...
          attributes[attribute.casefold()] = "" # For attribute like required / disabled
